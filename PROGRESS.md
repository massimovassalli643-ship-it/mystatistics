...
| Deployment attivi | Situazione al 20/09/2026 sera (da Gestisci deployment): attivi **"v5.3 - OCR numeri d…"** (quello usato dall'iPad: risponde v5.3) e due "v5 (13/09/2026)…" ancora alla versione 14 (v5.1) da allineare, vedi "Da verificare"; **"Senza titolo"** (ex deployment dell'iPad, v5.2) è stato **archiviato** e non risponde più; archiviati anche "v5.2 - ping GET c…" e le versioni v4.x. Un deployment archiviato non risponde: prima di archiviarne uno, controllare che non sia l'URL salvato sull'iPad |
...
- [ ] Portare alla stessa versione della v5.3 anche i due deployment "v5 (13/09/2026)…" ancora alla 14: Gestisci deployment → selezionare → matita → menu Versione → scegliere quella con descrizione "v5.3 - OCR numeri di maglia a mano" (senza creare nuove versioni) → Implementa. L'URL non cambia. Non è urgente (l'iPad usa già il deployment alla v5.3): serve solo se in futuro si usa uno di quegli URL. Alternativa: archiviarli, ma solo dopo aver verificato che nessun dispositivo abbia salvato il loro URL
